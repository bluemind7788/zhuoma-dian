...
app.get('/mobile', function(req, res, next) {
  //这里就是生成你默认页面的代码
  //下面这句的意思是用一个叫做index的模板生成页面
  res.sendfile("./website/mobile.html");
});
...
app.post('/image/uploading', function(req, res, next){
  ...
  form.parse(req, function(err, fields, files) {
    ...
    if(err){
      ...
      var idx = uploadedPath.lastIndexOf('/') == -1 ? uploadedPath.lastIndexOf('\\') : uploadedPath.lastIndexOf('/');
      imageName = uploadedPath.substring(idx+1, uploadedPath.lastIndexOf('.'))

    }

    // res.writeHead(200, {'content-type': 'text/plain;charset=utf-8'});
    // res.end(util.inspect({imgname: imageName, fields: fields, files: filesTmp}));
    res.json({imgname: imageName, fields: fields, files: filesTmp});
  });
});
